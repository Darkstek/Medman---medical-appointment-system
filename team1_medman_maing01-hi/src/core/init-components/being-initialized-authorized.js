...
const BeingInitializedAuthorized = createVisualComponent({
  ...
  render() {
    ...
    const { data: team1Medman } = useSubAppData();
    ...
    if (team1Medman?.temporaryData?.dtoIn.mode) {
      team1Medman.temporaryData.dtoIn.mode = "retry";
    }

    const [showConsole, setShowConsole] = useState(!!team1Medman.consoleCode);
    const [retryInputData, setRetryInputData] = useState(
      JSON.stringify({ force: false, mode: "retry", data: team1Medman?.temporaryData?.dtoIn }, null, 2),
    );
    const [rollbackInputData, setRollbackInputData] = useState(
      JSON.stringify({ force: true, mode: "rollback", data: {} }, null, 2),
    );
    const consoleBaseUri = team1Medman.temporaryData.progressMap.uuConsoleUri;
    const progressCode = team1Medman.temporaryData.progressMap.progressCode;
    const consoleCode = team1Medman.temporaryData.progressMap.consoleCode;
